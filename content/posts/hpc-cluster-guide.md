...
如果不删除影子文件为什么会报错？
  ...
  macOS 的文件系统（HFS+/APFS）会给每个文件额外生成一个以 ._ 开头的隐藏文件，叫 AppleDouble 
  影子文件。比如：

  data/
  ├── file-000.parquet       ← 真正的 Parquet 数据文件
  ├── ._file-000.parquet     ← macOS 自动生成的影子文件（Mac 的元数据）

  这些 ._ 文件里存的只是 Mac 的文件属性（比如 Finder 标签、创建者信息），内容完全不是 Parquet 
  格式，开头几个字节是 Mac 的私有格式标记。

  LeRobot 加载数据集时，会扫描 data/ 目录下所有 .parquet 结尾的文件。它看到 ._file-000.parquet 也以
  .parquet 结尾，就尝试用 Parquet 解析器去读它 —— 但里面的字节根本不是 Parquet 格式，所以报错：

  ▎ Parquet magic bytes not found

  解决方法

  find /gpfs1/scratch/xinyincai3 -name "._*" -delete
  find /gpfs1/scratch/xinyincai3 -name ".DS_Store" -delete
  ...
  │ find                      │ 搜索文件                                             │
  ├───────────────────────────┼──────────────────────────────────────────────────────┤
  │ /gpfs1/scratch/xinyincai3 │ 从这个目录开始往下搜索（包括所有子目录）             │
  ├───────────────────────────┼──────────────────────────────────────────────────────┤
  │ -name "._*"               │ 匹配所有以 ._ 开头的文件（* 是通配符，代表任意字符） │
  ├───────────────────────────┼──────────────────────────────────────────────────────┤
  │ -delete                   │ 找到后直接删除                                       │
  ├───────────────────────────┼──────────────────────────────────────────────────────┤
  │ -name ".DS_Store"         │ Mac Finder 生成的文件夹设置文件，也一并删掉          │
  └───────────────────────────┴──────────────────────────────────────────────────────┘

如果以后从 Mac 往任何 Linux服务器传文件，都建议传完后跑一下
---
